...
func _ready() -> void:
	var original_resource = load("res://shaders/jellyfish_npc_body.tres")
	...
	if npc_model != null:
		...
		match parent_color:
			0:
				npc_model.material_override.albedo_color = Color(1,0,0)
				print(npc_model.get_material_override().albedo_color)
			1:
				npc_model.get_material_override().albedo_color = Color(0,1,0)
				print(npc_model.get_material_override().albedo_color)
			2:
				npc_model.get_material_override().albedo_color = Color(0,0,1)
				print(npc_model.get_material_override().albedo_color)
